 ...
 *
 */
class SMD_Stylisthub_Adminhtml_OrderController extends Mage_Adminhtml_Controller_action {
	protected function _initAction() {
		$this->loadLayout ()->_setActiveMenu ( 'stylisthub/items' )->_addBreadcrumb ( Mage::helper ( 'adminhtml' )->__ ( 'Items Manager' ), Mage::helper ( 'adminhtml' )->__ ( 'Item Manager' ) );
		return $this;
	}
	/**
	...
	public function indexAction() {
		...
		if (empty ( $stylistId )) {
			$this->_redirect ( 'stylisthubadmin/adminhtml_orderview' );
			return;
		}
			...
			try {
				...
				if (empty ( $transaction_id )) {
					$Data = array (
							...
							'stylist_commission' => $stylist_commission,
	...
	public function masscreditAction() {
		...
		foreach ( $stylisthub as $value ) {
			$model = Mage::helper ( 'stylisthub/stylisthub' )->updateCredit ( $value );
			$stylist_id = $model->getStylistId ();
			$admin_commission = $model->getCommissionFee ();
			$stylist_commission = $model->getStylistAmount ();
			$order_id = $model->getOrderId ();
